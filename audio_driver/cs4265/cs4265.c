			...
			unsigned int freq, int dir)
{
	...
	struct cs4265_typedef *cs4265 = snd_soc_component_get_drvdata(component);
	int i;


	printk("cs4265_set_sysclk(): Here!\n");
	// nếu clk khác 0 là lõi
	if (clk_id != 0) {
...
static int cs4265_set_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	// cấu hình cho snd theo tình hình bit
...
static int cs4265_set_mutex(struct snd_soc_dai *dai, int mute)
{
	struct snd_soc_component *component = dai->component;


	if (mute) {
		// nếu được mutex thì ghi bit update mutexx
		printk("cs4265_set_mutex(): Mute On.\n");
		...
		printk("cs4265_set_mutex(): Mute Off.\n");
		snd_soc_component_update_bits(component, CS4265_DAC_CTL,
			CS4265_DAC_CTL_MUTE,
			0);
		snd_soc_component_update_bits(component, CS4265_SPDIF_CTL2,
...
static int cs4265_set_pcm_hw_params(struct snd_pcm_substream *substream,
				     struct snd_pcm_hw_params *params,
				     struct snd_soc_dai *dai)
{
	// khai báo struct quản lý âm thanh
	struct snd_soc_component *component = dai->component;
	// lấy ofdevice id struct của cs4265
	struct cs4265_typedef *cs4265 = snd_soc_component_get_drvdata(component);
	int index;

	printk("cs4265_set_pcm_hw_params(): Here!\n");

	// kiểm tra xem có phải djang stream và đúng format stream hay không
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE &&
		((cs4265->format & SND_SOC_DAIFMT_FORMAT_MASK)
		== SND_SOC_DAIFMT_RIGHT_J))
		return -EINVAL;

	//  lấy clock của cs4265 
	index = cs4265_get_clk_index(cs4265->sysclk, params_rate(params));
	if (index >= 0) {
		// cấu hình theo tần số cho cs42265
		snd_soc_component_update_bits(component, CS4265_ADC_CTL,
	...
	switch (cs4265->format & SND_SOC_DAIFMT_FORMAT_MASK) {
		// kiểm tra đnag truyền kiểu i2s 2 bên hay là mỗi moojet bên thôi
	case SND_SOC_DAIFMT_I2S:
		snd_soc_component_update_bits(component, CS4265_DAC_CTL,
...
static const struct snd_soc_dai_ops cs4265_ops = {
	.hw_params	= cs4265_set_pcm_hw_params, // hàm cấu hình hardware cho dai
	.digital_mute	= cs4265_set_mutex, // thực hiện chức nưng mutex
	.set_fmt	= cs4265_set_fmt, // thực hiện chức năng format
	.set_sysclk	= cs4265_set_sysclk, // thực hiện chức năng set clock cho audio i2s
};
...
/*implement functional for driver*/
static int cs4265_i2s_probe(struct i2c_client *i2c_client, const struct i2c_device_id *id) {
    struct cs4265_typedef *cs4265;
    int ret = 0;
	unsigned int devid = 0;
	unsigned int reg;

    cs4265 = devm_kzalloc(&i2c_client->dev, sizeof(struct cs4265_typedef), GFP_KERNEL);
	if (cs4265 == NULL)
		return -ENOMEM;
    ...
    cs4265->reset_gpio = devm_gpiod_get_optional(&i2c_client->dev, "reset", GPIOD_OUT_LOW);
    ...
    printk(KERN_ALERT "cs4265_i2s_probe(): Querying I2s for cs4265 ID.\n");
    ...
    printk(KERN_ALERT "cs4265_i2s_probe(): cs4265 found.\n");

    /*get info*/
    dev_info(&i2c_client->dev, "CS4265 Version %x\n", reg & CS4265_REV_ID_MASK);
...
/*configure driver*/
static struct i2c_driver cs4265_i2s_driver = {
    .driver = {
        .name = "cs4265_test_by_Bhien",
        .of_match_table  = cs4265_of_match,
    },
    .id_table = cs4265_id,
    .probe =    cs4265_i2s_probe,
};
module_i2c_driver(cs4265_i2s_driver);


MODULE_DESCRIPTION("Driver test for CS4265");
MODULE_AUTHOR("Bui Hien (Mark) buihien29112002@gmail.com");
MODULE_LICENSE("GPL");