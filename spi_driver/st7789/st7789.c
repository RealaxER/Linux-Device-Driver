...
struct spi_device *spi_init;
unsigned spi0_cmd, spi0_rst;
char ST7789_SPI_PORT;

static int ili9255_write_regs(u8 *buf, int len) {
    int ret;
    unsigned char *txdata;
    struct spi_message m;
    struct spi_transfer *t;
    /* implement spi_transfer */
	...
	if(!txdata) {
		kfree(txdata);				/* free stack memory*/
	}
    /*config tx_data for trasmit*/
    memcpy(&txdata[0], buf, len);	/*copy register to buffer*/
    t->tx_buf = txdata; 
    t->len = len; /*address 1 byte and data 1 byte*/
    spi_message_init(&m); //init spi_message
    spi_message_add_tail(t,&m); //add spi_mesage to transper for transmit
    ret = spi_sync(spi_init,&m);   // sync to spi master 
    if(ret < 0) {
