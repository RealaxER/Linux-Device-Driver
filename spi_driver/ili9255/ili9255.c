...
static void ili9255_write_onereg(uint8_t reg, uint8_t len) {
    ili9255_write_regs(&reg, 1);
}
/*
...
void spi_write_ili9255(unsigned char data) {
	...
	HAL_SPI_Transmit(hspi_init, data, 1, 100);
}
...
void draw_bitmap(int x1, int y1, int scale, const unsigned int *bmp) {
	...
	//If landscape view then translate everyting -90 degrees
	if (LANDSCAPE) {
		swap_char((char*)&x1,(char*)&y1);
		swap_char((char*)&x2, (char*)&y2);
		y1 = WIDTH - y1;
		y2 = WIDTH - y2;
		swap_char((char*)&y2, (char*)&y1);
